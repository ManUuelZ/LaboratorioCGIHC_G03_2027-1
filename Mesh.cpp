...
void MeshColor::CreateMeshColor(GLfloat *vertices, unsigned int numOfVertices)
{
	...
	glGenBuffers(1, &VBO);
	glBindBuffer(GL_ARRAY_BUFFER, VBO);
	glBufferData(GL_ARRAY_BUFFER, sizeof(vertices[0]) * numOfVertices * 6 , vertices, GL_STATIC_DRAW); //pasarle los datos al VBO asignando tamaño, los datos y en este caso es estático pues no se modificarán los valores
...
void Mesh::RenderMeshGeometry()
{
	...
	glBindVertexArray(VAO);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, IBO);
	glDrawElements(GL_TRIANGLE_FAN, indexCount, GL_UNSIGNED_INT, 0); // Triangle fan -> primer vértice que se guarde, lo comparten todos los triángulos
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
	glBindVertexArray(0);
}
